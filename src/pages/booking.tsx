    ...
    <>
      ...
      <main className="flex min-h-screen items-center justify-center bg-gradient-to-b from-[#2e026d] to-[#15162c]">
        <form className="flex flex-col gap-3 w-1/2 px-9">
          <input onChange={e => setPatientName(e.target.value)} type="text" name="name" placeholder="Patient Name" className="p-3 rounded-lg" />
          <input onChange={e => setPhone(e.target.value)} type="text" name="phone" placeholder="Patient Name" className="p-3 rounded-lg" />
          ...
          <select className="p-3 rounded-lg" defaultValue={'123'} onChange={setDoctor}>
            ...
            {
              allDoctors.data?.map(doc => <option value={doc.id}>Dr. {doc.name}</option>)
            }
          </select>
          <button
